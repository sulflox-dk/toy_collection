...
const CollectionUi = {
    ...
    switchView(mode) {
        ...
        if (window.CollectionManager && typeof CollectionManager.loadPage === 'function') {
    ...
    renderGrid(data) {
        ...
        const { toys, pagination, view_mode } = data;
        ...
        // Render toys based on view mode
        if (toys && toys.length > 0) {
            if (this.viewMode === 'cards') {
                this.renderCardsView(toys);
            } else {
                this.renderListView(toys);
            }
     ...
     */
    createToyCard(toy) {
        const imageSrc = toy.main_image || toy.stock_image || '/assets/images/no-image.png';
        const statusBadge = this.getStatusBadge(toy.acquisition_status);
        const completenessBadge = this.getCompletenessBadge(toy.completeness_grade);
    ...
    refreshItem(id, data) {
        ...
        if (!oldEl) {
            oldEl = document.querySelector(`.toy-card[data-id="${id}"], tr[data-id="${id}"]`);
        }
        ...
        if (view_mode === 'cards' || oldEl.classList.contains('toy-card')) {
            ...
            if (tempTr.firstElementChild && tempTr.firstElementChild.tagName === 'TR') {
    ...
    removeItem(id) {
        const element = this.container 
            ...
            : document.querySelector(`.toy-card[data-id="${id}"], tr[data-id="${id}"]`);
