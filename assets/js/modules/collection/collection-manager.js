...
class CollectionManager extends EntityManager {
	...
	setupFilterElements() {
		this.filterElements = {
			...
			entSource:
				document.getElementById('filterEntSource') ||
	...
	async deleteToyItem(itemId, btnElement) {
		const confirmed = await UiHelper.confirmDelete(
			'this item from your collection',
		);
	...
	async loadPage(page) {
		...
		try {
			...
			const html = await CollectionApi.getAll(filters, page);

			// Insert HTML directly
			CollectionUi.renderGrid(html);
		} catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
	...
	App.deleteToyItem = (itemId, btnElement) =>
		collectionManagerInstance.deleteToyItem(itemId, btnElement);
