...
const CollectionApi = {
    ...
    async getAll(filters = {}, page = 1) {
        ...
        // Response format: { success: true, data: { toys: [], pagination: {}, view_mode: '' } }
        if (!response.success) {
            throw new Error(response.message || 'Failed to load toys');
        }
     ...
     */
    async getById(id) {
        const url = ApiClient.buildModuleUrl('Collection', 'Toy', 'get_item_html');
        const response = await ApiClient.get(url, { id, t: new Date().getTime() });
        
        // Response format: { success: true, data: { toy: {}, view_mode: '' } }
        if (!response.success) {
            throw new Error(response.message || 'Failed to load toy');
        }
    ...
    async getMasterToyItems(masterToyId) {
        const url = ApiClient.buildModuleUrl('Collection', 'Api', 'get_master_toy_items');
