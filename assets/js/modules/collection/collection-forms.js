...
const CollectionForms = {
	...
	attachEventHandlers() {
		...
		if (universeSelect) {
			universeSelect.addEventListener('change', (e) =>
				this.loadManufacturers(e.target.value),
			);
		}

		if (manufacturerSelect) {
			manufacturerSelect.addEventListener('change', (e) =>
				this.loadLines(e.target.value),
			);
		}

		if (lineSelect) {
			lineSelect.addEventListener('change', (e) =>
				this.loadToys(e.target.value),
			);
		}
	...
	initializeWidget() {
		...
		if (widgetWrapper.dataset.allToys) {
			try {
				this.currentMasterToysList = JSON.parse(
					widgetWrapper.dataset.allToys,
				);
			} catch (e) {
	...
	initializeItemRows() {
		...
		if (container.dataset.masterToyItems) {
			try {
				this.availableMasterToyItems = JSON.parse(
					container.dataset.masterToyItems,
				);
			} catch (e) {
				...
				if (contentType && contentType.includes('application/json')) {
					...
					if (modalContent) {
						...
						if (
							...
							typeof CollectionMedia.init === 'function'
						) {
	...
	async loadManufacturers(universeId) {
		...
		try {
			const manufacturers =
				await window.manufacturerManager.getByUniverse(universeId);
			this.populateSelect(
				manufacturerSelect,
				manufacturers,
				'Select Manufacturer',
	...
	async loadLines(manufacturerId) {
		...
		try {
			const lines =
				await window.toyLineManager.getByManufacturer(manufacturerId);
	...
	async loadMasterToyItems(masterToyId) {
		try {
			this.availableMasterToyItems =
				await CollectionApi.getMasterToyItems(masterToyId);
			this.refreshExistingRows();

			// Update dataset for new rows
			const container = document.getElementById('childItemsContainer');
			if (container) {
				container.dataset.masterToyItems = JSON.stringify(
					this.availableMasterToyItems,
				);
			}
		} catch (error) {
	...
	addAllItemsFromMaster() {
		...
		if (masterItems.length === 0) {
			UiHelper.showWarning(
				'No items defined for this Master Toy in the catalog.',
			);
			return;
		}
	...
	refreshExistingRows() {
		...
		selects.forEach((select) => {
			...
			let options =
				this.availableMasterToyItems.length > 0
			...
			if (
				currentVal &&
				this.availableMasterToyItems.some((p) => p.id == currentVal)
			) {
	...
	handleSaveSuccess(data) {
		...
		if (
			...
			typeof CollectionManager.refreshAfterSave === 'function'
		) {
