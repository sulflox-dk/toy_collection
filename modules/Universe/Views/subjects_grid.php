        ...
        <tbody>
            <?php foreach($data as $s): ?>
                <tr data-json='<?= json_encode($s, JSON_HEX_APOS) ?>'>
                    ...
                    <td class="small"><span class="badge bg-light text-dark border"><?= $s['type'] ?></span></td>
                    <td class="small text-muted"><?= htmlspecialchars($s['faction'] ?? '-') ?></td>
                    <td class="text-center">
                        <span class="badge bg-light text-dark border" title="Catalog Items linked">
                            <?= $s['usage_count'] ?>
                        </span>
                    </td>
        ...
        <nav>
            <ul class="pagination pagination-sm mb-0">
                <li class="page-item <?= $current_page <= 1 ? 'disabled' : '' ?>">
                    <a class="page-link" href="#" onclick="SubjectMgr.loadPage(<?= $current_page - 1 ?>); return false;">&laquo;</a>
                </li>
                <?php for($i = 1; $i <= $pages; $i++): ?>
                    <li class="page-item <?= $i == $current_page ? 'active' : '' ?>">
                        <a class="page-link" href="#" onclick="SubjectMgr.loadPage(<?= $i ?>); return false;"><?= $i ?></a>
                    </li>
                <?php endfor; ?>
                <li class="page-item <?= $current_page >= $pages ? 'disabled' : '' ?>">
                    <a class="page-link" href="#" onclick="SubjectMgr.loadPage(<?= $current_page + 1 ?>); return false;">&raquo;</a>
                </li>
            </ul>
        </nav>
    </div>
