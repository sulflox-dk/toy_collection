...
class ToyModel {
    ...
    public function saveItems(int $parentId, array $items) {
        ...
        foreach ($items as $item) {
            ...
            // Forbered data array (fælles for create/update)
            $data = [
        ...
        if (!empty($idsToDelete)) {
            // Brug deleteItem metoden så vi også får slettet billed-relationer!
    ...
    public function delete(int $id) {
        ...
        foreach ($items as $item) {
            ...
            foreach ($mediaIds as $mid) {
                // A. Slet f�rst relationen i map-tabellen (Vigtigt n�r du ikke bruger CASCADE!)
                $this->db->query("DELETE FROM collection_toy_item_media_map WHERE media_file_id = :mid", ['mid' => $mid]);
                
                // B. Slet selve filen og media_files r�kken
                $mediaModel->delete($mid);
            }

            // C. Slet selve item-r�kken i databasen (Dataen)
        ...
        // 3. SLET PARENT TOY DATA (Hoved-r�kken)
